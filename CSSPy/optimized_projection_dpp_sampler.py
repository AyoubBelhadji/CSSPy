...
class optimized_projection_DPP_Sampler:
    def __init__(self, A, k,D, Q,N):
        self.A = A
        self.D = D
        self.nnz_D = np.count_nonzero(D)
        self.eig_val = np.zeros(self.nnz_D)
        self.eig_val[0:k] = np.ones(k)
    ...
    def MultiRounds(self):
        sampled_indices_ = k_dpp_KuTa12(self.eig_val, self.Q, self.k)
